  ...
  return (
    <aside className="w-56 sm:w-60 bg-[#121212] border-r border-white/5 flex flex-col shrink-0 select-none h-full">
      ...
      <div className="h-16 px-5 flex items-center gap-3">
        <div className="w-7 h-7 rounded-full bg-[#1db954] flex items-center justify-center shrink-0 shadow-sm">
          <svg className="w-4 h-4 text-black fill-current" viewBox="0 0 24 24">
            <path d="M12 2C6.477 2 2 6.477 2 12s4.477 10 10 10 10-4.477 10-10S17.523 2 12 2zm4.586 14.424c-.18.295-.563.387-.857.207-2.35-1.434-5.308-1.758-8.793-.963-.335.077-.67-.133-.746-.468-.077-.334.132-.67.467-.746 3.808-.87 7.076-.496 9.722 1.115.294.18.386.562.207.855zm1.226-2.73c-.226.367-.708.482-1.075.257-2.69-1.653-6.79-2.131-9.97-1.165-.413.125-.85-.108-.975-.522-.125-.414.108-.85.522-.975 3.637-1.104 8.163-.568 11.24 1.328.368.226.483.708.258 1.077zm.106-2.847C14.692 8.95 9.375 8.775 6.297 9.71c-.494.15-1.018-.128-1.168-.622-.15-.494.128-1.018.622-1.168 3.532-1.072 9.404-.866 13.115 1.337.445.264.59.838.327 1.282-.264.444-.838.59-1.282.327z"/>
          </svg>
        </div>
          ...
          {playlists.map((pl) => {
            ...
            return (
              ...
              >
                ...
                {pl.id.startsWith('playlist-') && !['playlist-mexican-pride', 'playlist-junior-h', 'playlist-corridos-belicos', 'playlist-tiktok-2026', 'playlist-today-top-hits', 'playlist-chill-late-night'].includes(pl.id) && (
                  <button
                    type="button"
                    onClick={(e) => {
                      e.stopPropagation();
